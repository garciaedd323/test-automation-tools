...
- [maxParallelForks y junit-platform.properties](./selenium/notas/maxparallelforks-junit-platform-properties.md) — dos paralelismos distintos (procesos vs hilos), y por qué uno de ellos no hace nada si tu suite usa runners clásicos de Cucumber.
- [Git worktree para correr Serenity en paralelo sin choques](./selenium/notas/git-worktree-paralelismo-serenity.md) — qué hacer cuando aislar procesos no basta porque el checkout sigue siendo compartido.
...
- [maxParallelForks y junit-platform.properties](./selenium/notas/maxparallelforks-junit-platform-properties.md) — paralelismo entre procesos (forks de Gradle) vs paralelismo entre hilos (motor Jupiter de JUnit 5), por qué `junit-platform.properties` no afecta a runners clásicos `@RunWith(CucumberWithSerenity.class)` (motor Vintage los trata como un descriptor opaco), qué cambiaría al migrar a `cucumber-junit-platform-engine`, riesgos de WebDriver no thread-safe y datos de prueba compartidos, y comparación con Selenium Grid. Incluye tabla comparativa y analogías cotidianas.
- [Git worktree para correr Serenity en paralelo sin choques](./selenium/notas/git-worktree-paralelismo-serenity.md) — por qué correr 2+ invocaciones de `gradlew` contra el mismo checkout choca en capas sucesivas (carpeta binaria de resultados de Gradle, carpeta de agregación de Serenity ignorando `-Dserenity.outputDirectory`, caché de up-to-date de Gradle), cómo `git worktree add --detach` separa el checkout físicamente sin duplicar el historial, qué se comparte y qué no (y por qué los worktrees no se actualizan solos), archivos gitignored que hay que copiar a mano (`chromedriver.exe`, `.env`), orquestación manual y con script, y por qué depurar un escenario suelto debe hacerse desde el proyecto original y no "parchando" el reporte de un worktree. Incluye tabla resumen de comandos y comparación con las notas de maxParallelForks y Selenium Grid.
...
- [Troubleshooting: configurar Appium en Windows desde cero](./appium/notas/troubleshooting-appium-windows.md) — cadena completa de errores típicos al levantar el entorno por primera vez: Appium no reconocido como comando, falta de drivers, IntelliJ sin botón de ejecución (Power Save Mode), `aapt2.exe` no encontrado por SDKs duplicados, espacios ocultos en variables de entorno, y bloqueos de Gradle por tener el proyecto dentro de OneDrive. Incluye tabla síntoma→causa real y checklist de configuración.
...
- [x] Appium: troubleshooting de entorno en Windows agregado — nota nueva cubriendo los errores más comunes al configurar Appium + UiAutomator2 + Serenity/Cucumber + IntelliJ desde cero en Windows (comando no reconocido, drivers, `aapt2.exe`, variables de entorno, Power Save Mode, proyectos dentro de OneDrive).
